...
class MLP: #(Fully connected FFNN)
    ...
    def __init__(self, model_nametag = 'a MLP', model_structure = '784:identity|100:tanh|10:softmax', learning_rate = 0.005, encoding = 'one-hot', load_model_np = None):
        ...
        * MLP 초기화 입력변수 *
        ...
        if load_model_np is None:
            ...
            activation_available = set(self.id_activations).issubset(['identity','relu','sigmoid','selu','softmax','tanh','exp-1'])
            ...
            for i in range(self.n_layers-1): # i = 0 .. n_layers-2
                if self.id_activations[i] in ['identity','selu','sigmoid','softmax','tanh','exp-1']: # Xavier-init.
    ...
    def backpropagate(self, target_list, error_func, l_left = None, l_right = None, train_subnet = False):
        ...
        if self.error_func == 'mse':
            ...
            self.dE_df = (self.output_y - target_y)
        ...
        for i in range(self.n_layers-1, 0, -1):
            ...
            if i == self.n_layers-1: #rev@1209 
                if self.id_activations[i] == 'softmax':
                    self.delta[i] = np.matmul(self.df_da(self.a[i], func=self.id_activations[i]), self.dE_df).reshape(len(self.dE_df),1)
                else:
        ...
        if train_subnet == False:
            L_right, L_left = self.n_layers-1, 0

        elif train_subnet == True:
            L_right, L_left = l_right, l_left
    ...
    def train(self, input_list, target_list, error_func, l_left=None, l_right=None, train_subnet=False):
        """
        * Train networks given data & label
        ...
        E = self.backpropagate(target_list, error_func, l_left, l_right, train_subnet) 
...
class AE(MLP): #rev@1211
    """
    * Auto Encoder Network
    1> E: Encoder
    2> D: Decoder
    3> ED: Encoder + Decoder
    """

    # dim of encoder output (input_dim of decoder)
    latent_dim = None

    # dim of real data features (input_dim of encoder & output_dim of decoder)
    data_dim = None

    def __init__(self,
            n_Z = 256, n_X = 784,
            model_structure_E = '784:identity|520:selu|256:sigmoid',
            model_nametag_E = 'AE_encoder', learning_rate_E = 0.001, encoding_E = 'float',
            model_structure_D = '256:identity|520:selu|784:sigmoid',
            model_nametag_D = 'AE_decoder', learning_rate_D = 0.001, encoding_D = 'float'):
        ...
        # creating an encoder
        self.E = MLP(
                model_structure = model_structure_E, \
                model_nametag = model_nametag_E, \
        ...
        # creating a combined network (stacked E(x) + D(E(x)))
        self.get_NN_stacked(self.E, self.D)

        return


    def get_NN_stacked(self,E,D):

        model_structure_ED = E.model_structure + D.model_structure.lstrip(D.model_structure.split('|')[0]) 

        learning_rate_ED = E.learning_rate 
        model_nametag_ED = 'AE_ED_Stacked_NN['+model_structure_ED+']_LR:'+str(learning_rate_ED)
        encoding_ED = "float"
       
        self.ED = MLP(
                model_structure = model_structure_ED, \
                model_nametag = model_nametag_ED, \
                learning_rate = learning_rate_ED, \
                encoding = encoding_ED)
        ...
        self.ED.a = E.a + D.a[1:]
        self.ED.f = E.f + D.f[1:]
        self.ED.delta = E.delta + D.delta[1:]

        return 


    def get_E_updated(self):

        self.E.W = self.ED.W[:self.E.n_layers-1]
        self.E.B = self.ED.B[:self.E.n_layers-1]
    ...
    def get_D_updated(self):
        ...
        self.D.W = self.ED.W[self.E.n_layers-1:]
        self.D.B = self.ED.B[self.E.n_layers-1:]

        return


    def check_performance(self, data_list, id_data_min, id_data_max, train=True):
        """
        * 정확도와 평균에러값 반환 
        * 입력: ( data_list, id_data_min=0, id_data_max=len(data)-1, data_type=None )
        > data_list: numpy_array in shape of (n_samples, [label, x1, x2, ...])
        > id_data_min: 시작 데이터 아이디 (0,...)
        > id_data_max: 마지막 데이터 아이디 (...len(data)-1)
        > train: True(False) for train(test) data input
        """
        # 예측 정확도 스코어 리스트, 에러 리스트 초기화
        score_list, error_list = [], []
        ...
        for data in data_list:
            ...
            # 출력값
            y_predicted = self.ED.feedforward(X) # (n_output_nodes, 1)

            # 오차값의 계산 (Mean Squared Error의 경우) 
            error = 0.5*((y_target.reshape(-1) - y_predicted.reshape(-1))**2).mean()
        ...
        error_list_np = np.asarray(error_list)
        error_mean = error_list_np.mean()


        # record the error value in self.history['error']
        if train == True:
            self.history['error'].append(error_mean)
    
            print (" ")
            print (" * 평균에러(훈련샘플) = ", error_mean)

        elif train == False:
            self.history['val_error'].append(error_mean)

            print (" ")
            print (" * 평균에러(테스트샘플) = ", error_mean)


        else:
            raise ValueError(' => only True / False for the arg : data_used_for_train ')
